  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        // Safely check WebGL context to prevent 'Cannot read properties of null (reading alpha)' crash
        const gl = this.renderer.getContext();
        if (!gl || !gl.getContextAttributes || !gl.getContextAttributes()) {
          console.warn("WebGL context or attributes creation failed. Hyperspeed canvas will be skipped.");
      ...
      initPasses() {
        ...
        const gl = this.renderer.getContext();
        if (!gl || !gl.getContextAttributes || !gl.getContextAttributes()) {
          console.warn("WebGL context or attributes lost before initPasses.");
          this.webglSupported = false;
          return;
        }
      ...
      init() {
        ...
        try {
          ...
          console.warn("WebGL initialization failed during init(). Graceful fallback applied.", error);
          this.webglSupported = false;
    ...
    const roadMarkings_fragment = `
      vUvCopy.y = mod(vUvCopy.y + uTime * 0.05, 1.);
      float laneWidth = 1.0 / uLanes;
      float brokenLineWidth = laneWidth * uBrokenLinesWidthPercentage;
      float laneEmptySpace = 1. - uBrokenLinesLengthPercentage;

      float brokenLines = step(1.0 - brokenLineWidth, fract(vUvCopy.x * 2.0)) * step(laneEmptySpace, fract(vUvCopy.y * 10.0));
      float sideLines = step(1.0 - brokenLineWidth, fract((vUvCopy.x - laneWidth * (uLanes - 1.0)) * 2.0)) + step(brokenLineWidth, vUvCopy.x);

      brokenLines = mix(brokenLines, sideLines, vUvCopy.x);
    `;
